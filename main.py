...
CONFIG_TABLE = "wfm_config"   # create if not exists (SQL below)
CONFIG_ID = "default"
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "break_pattern": [15, 30, 15],# minutes: 15 / 30 / 15
    ...
    # stub for later (not enforced in M3)
    "site_hours": {
...
def get_config() -> Dict[str, Any]:
    ...
    # fetch overrides from Supabase if table exists
...
def set_config(overrides: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not REST_BASE:
        # No persistence; just update cache
...
# -----------------------------------------------------------------------------
# Requirements (now config-driven defaults)
# -----------------------------------------------------------------------------
...
def generate_roster(req: RosterRequest):
    ...
    BP = [int(x) for x in cfg["break_pattern"]]  # [15,30,15] etc.
    ...
    for item in roster:
        ...
        b1_target = clamp(lunch_target - LUNCH_GAP - 45, place_start, place_end)
        b3_target = clamp(lunch_target + LUNCH_GAP + 45, place_start, place_end)
    ...
    summary_intervals = []
    for t in times:
        # derive how many are on break at time t
        onbreak = 0
        for it in roster:
            for b in it.get("breaks", []):
                bs = time_to_minutes(b["start"] + ":00") if len(b["start"]) == 5 else time_to_minutes(b["start"])
                be = time_to_minutes(b["end"] + ":00") if len(b["end"]) == 5 else time_to_minutes(b["end"])
                if bs <= t < be:
                    onbreak += 1
    ...
    return {
        ...
        "roster": roster,
        "notes": [
            "Greedy 9h assignment; fairness cap; site hours stub; 12h rest across days coming next",
            "Config-driven: shift_minutes, break_pattern, caps, targets, shrinkage, etc."
        ],
    }
