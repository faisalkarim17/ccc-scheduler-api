...
def generate_roster(req: RosterRequest):
    ...
    try:
        # Build a full 30-min grid over the rostered day
        all_starts, all_ends = [], []
        for it in roster:
        ...
        if not all_starts:
            return {
                "date": req.date,
                "intervals": [
                    {"time": minutes_to_hhmm(t), "req": demand[t], "assigned": assigned[t]}
                    for t in times
                ],
                "agents_used": len(used),
                "roster": [],
                "notes": ["No rostered agents; breaks skipped."],
            }
        ...
        # Fairness: cap % of agents on break per interval (>=1 if anyone works)
        BREAK_CAP_FRAC = 0.25
    ...
    summary = {
        ...
        "notes": [
            ...
            "Next: subtract breaks from capacity, multi-day rest rules, site hours.",
        ],
    }
    return summary
